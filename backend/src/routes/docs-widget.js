// Auto-generated feature route: docs-widget
// Domain: embeddable docs widget service — Capability: powering embedded KB search and tracking usage
// Mounted under: /api/docs-widget
...
// POST /api/docs-widget/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a embeddable docs widget service. Your task is powering embedded KB search and tracking usage. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/docs-widget/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/docs-widget/:id — fetch one
...
// DELETE /api/docs-widget/:id — discard a run
