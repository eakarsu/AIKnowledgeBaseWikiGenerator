// Auto-generated feature route: rag-qa
// Domain: RAG knowledge assistant — Capability: answering questions from KB corpus with source citations
// Mounted under: /api/rag-qa
...
// POST /api/rag-qa/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a RAG knowledge assistant. Your task is answering questions from KB corpus with source citations. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/rag-qa/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/rag-qa/:id — fetch one
...
// DELETE /api/rag-qa/:id — discard a run
