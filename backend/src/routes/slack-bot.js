// Auto-generated feature route: slack-bot
// Domain: Slack/Teams KB bot handler — Capability: answering in-channel KB lookups
// Mounted under: /api/slack-bot
...
// POST /api/slack-bot/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a Slack/Teams KB bot handler. Your task is answering in-channel KB lookups. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/slack-bot/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/slack-bot/:id — fetch one
...
// DELETE /api/slack-bot/:id — discard a run
