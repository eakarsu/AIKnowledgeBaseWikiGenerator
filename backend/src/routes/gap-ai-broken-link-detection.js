...
// Route: /api/gap-ai-broken-link-detection
// Feature: Ai Broken Link Detection
// Description: (orphan articles / dead internal links)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Broken Link Detection. (orphan articles / dead internal links)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Broken Link Detection\nDescription: (orphan articles / dead internal links)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-broken-link-detection', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-broken-link-detection', title: 'Ai Broken Link Detection', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-broken-link-detection', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-broken-link-detection']);
      return res.json({ slug: 'ai-broken-link-detection', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-broken-link-detection', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-broken-link-detection', items: [], error: (e && e.message) || 'Server error' });
  }
});
