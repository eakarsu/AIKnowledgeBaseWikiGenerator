...
// Route: /api/gap-ai-related-article-clustering
// Feature: Ai Related Article Clustering
// Description: (semantic similarity / dedupe)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Related Article Clustering. (semantic similarity / dedupe)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Related Article Clustering\nDescription: (semantic similarity / dedupe)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-related-article-clustering', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-related-article-clustering', title: 'Ai Related Article Clustering', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-related-article-clustering', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-related-article-clustering']);
      return res.json({ slug: 'ai-related-article-clustering', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-related-article-clustering', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-related-article-clustering', items: [], error: (e && e.message) || 'Server error' });
  }
});
