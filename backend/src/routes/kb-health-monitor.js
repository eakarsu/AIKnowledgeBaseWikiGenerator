// Auto-generated feature route: kb-health-monitor
// Domain: KB hygiene agent — Capability: triaging stale, broken, or orphaned articles
// Mounted under: /api/kb-health-monitor
...
// POST /api/kb-health-monitor/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a KB hygiene agent. Your task is triaging stale, broken, or orphaned articles. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/kb-health-monitor/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/kb-health-monitor/:id — fetch one
...
// DELETE /api/kb-health-monitor/:id — discard a run
