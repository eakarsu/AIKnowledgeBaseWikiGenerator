// Auto-generated feature route: multi-language-kb
// Domain: translation + cultural adaptation expert — Capability: producing localized KB content with cultural context
// Mounted under: /api/multi-language-kb
...
// POST /api/multi-language-kb/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a translation + cultural adaptation expert. Your task is producing localized KB content with cultural context. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/multi-language-kb/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/multi-language-kb/:id — fetch one
...
// DELETE /api/multi-language-kb/:id — discard a run
