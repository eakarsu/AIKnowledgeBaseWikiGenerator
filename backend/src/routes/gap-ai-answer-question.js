...
// Route: /api/gap-ai-answer-question
// Feature: Ai Answer Question
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Answer Question. (RAG question answering)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Answer Question\nDescription: (RAG question answering)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-answer-question', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-answer-question', title: 'Ai Answer Question', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-answer-question', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-answer-question']);
      return res.json({ slug: 'ai-answer-question', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-answer-question', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-answer-question', items: [], error: (e && e.message) || 'Server error' });
  }
});
