...
// Route: /api/gap-audience
// Feature: Audience
// Description: Audience segmentation (role-based KB views)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Audience. Audience segmentation (role-based KB views)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'audience', title: 'Audience', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'audience', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['audience']);
      return res.json({ slug: 'audience', items: r.rows || [] });
    }
    return res.json({ slug: 'audience', items: [] });
  } catch (e) {
    return res.json({ slug: 'audience', items: [], error: (e && e.message) || 'Server error' });
  }
});
